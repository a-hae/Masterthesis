...
seventyTrue <- function(x) {
  if ((sum(x==TRUE)/length(x)) >= 0.70){
    return (TRUE)
  } else{ 
    return (FALSE)
  }
                    ...
                    return_features = FALSE, saga_lib)
{
  ...
  # geometry off debris flow
  sp.pred <- raster::rasterToPolygons(pred_thres, n = 4, dissolve = TRUE, na.rm=TRUE)
  geom_pred <- runoutGeom(sp.pred, elev = dem)
  geom_act <- runoutGeom(slide_poly_single, elev = dem)
  ...
  if(return_features){
    return(
      list(id = slide_id,
           conn = is_conn,
           roc = roc,
           length.pred = geom_pred$length,
           length.obs = geom_act$length,
           width.pred = geom_pred$width,
...
pcmGetConnOpt <- function(x, performance = "relerr", measure = "median", conn_proportion,
...
pcmSpConn_nm <- function(dem, slide_plys, slide_src, slide_id = 1, conn_obj,
                         rw_slp = 33, rw_ex = 3, rw_per = 2,
                         mu_grid, md_grid,
                         buffer_ext = 500, buffer_source = 50, gpp_iter = 1000,
                         predict_threshold = 0.5, plot_eval = FALSE,
                         return_features = FALSE, save_res = FALSE, saga_lib, com_nm)
{
...
pcmSpGrid <- function(chm_class, pcmmu_vec,dem, slide_plys, slide_src, slide_id = 1, conn_obj,
                      rw_slp = 33, rw_ex = 3, rw_per = 2,
                      chm,
                      buffer_ext = 500, buffer_source = 50, gpp_iter = 1000,
                      predict_threshold = 0.5, plot_eval = FALSE,
                      return_features = FALSE, save_res = FALSE, saga_lib) {
  ...
  for (m1 in pcmmu_vec) {
    for (m2 in pcmmu_vec){
      for (m3 in pcmmu_vec) {
        for (m4 in pcmmu_vec) {
          for (m5 in pcmmu_vec) {
            # difine mu variable for each forest class
            chm$mu[chm$stolla_crown_hgt_reclass == chm_class[1]] <- m1
            chm$mu[chm$stolla_crown_hgt_reclass == chm_class[2]] <- m2
            chm$mu[chm$stolla_crown_hgt_reclass == chm_class[3]] <- m3
            chm$mu[chm$stolla_crown_hgt_reclass == chm_class[4]] <- m4
            chm$mu[chm$stolla_crown_hgt_reclass == chm_class[5]] <- m5
            
            com <- paste("spmu" ,m1, m2, m3, m4, m5, sep = "_" )
            
            
            res[[counter]] <- pcmSpConn_nm(dem, slide_plys = runout_polygons, slide_src = src_poly, slide_id = poly_id, conn_obj = river,
                                           rw_slp = rw_opt$rw_slp_opt, rw_ex = rw_opt$rw_exp_opt, rw_per = rw_opt$rw_per_opt,
                                           mu_grid = chm$mu, md_grid = chm$md,
                                           buffer_ext = 5000, buffer_source = NULL, gpp_iter = 1000,
                                           predict_threshold = 0.5, plot_eval = FALSE,
                                           return_features = FALSE, save_res = TRUE, saga_lib = saga, com_nm = com)
            
            counter <- counter + 1
            #assign(paste("spmu" ,m1, m2,m3, m4, m5, sep = "_" ), ind_perf)
            # 
            #file_name <- paste("spmu" ,m1, m2,m3, m4, m5, ".Rd", sep = "_" )
            # 
            #save(ind_perf, file = file_name)
  ...
  if(save_res){
    save(res, file = paste("pcmSPconn_id_", slide_id, ".Rd", sep=""))
    #save(result_pcm, file = paste("result_pcmconn_gridsearch_", slide_id, ".Rd", sep=""))
  }
