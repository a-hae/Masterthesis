...
# Test PCM conn
# for(i in 3:length(runout_polygons)){
#   pcmConn(dem, slide_plys = runout_polygons, src_poly, slide_id = i, conn_obj = river,
#           rw_slp = 40, rw_ex = 3, rw_per = 1.6,
#           pcm_mu = 0.1, pcm_md = 40,
#           buffer_ext = 500, buffer_source = NULL, gpp_iter = 10,
#           predict_threshold = 0.5, plot_eval = TRUE,
#           return_features = FALSE, saga)
# 
# }
...
polyid_vec <- 3:4 #length(runout_polygons)
...
pcm_gridsearch_multi <-
  foreach(poly_id=polyid_vec, .packages=c('rgdal','raster', 'rgeos', 'ROCR', 'Rsagacmd', 'sf', 'runoptGPP')) %dopar% {
    ...
    pcmConnGridsearch(dem, conn_obj = river,
                      slide_plys = runout_polygons, slide_src = src_poly, slide_id = poly_id,
