  ...
  for (s in 1:length(slidesSHP)) {
    for  (k in 1:length(slidesSHP)){
      if (slidesSHP[s,]$Id != slidesSHP[k,]$Id)  {
        ...
        #}
        touch <- gTouches(slidesSHP[s,], slidesSHP[k,])
        overlap <- gOverlaps(slidesSHP[s,], slidesSHP[k,])
        if (touch == TRUE || overlap == TRUE) {
          ...
          if (slidesSHP[s,]$aggregate == 0 && slidesSHP[k,]$aggregate == 0){
            ...
            slides_con@data[slides_con@data$aggregate == slidesSHP[k,]$aggregate, "aggregate"] <- slidesSHP[s,]$aggregate
...
# 
# # merge all slides with same number
# slides_merged_fin <- aggregate(slides_merged_fin, by = "aggregate")
# 
# # create new ID column
# slides_merged_fin$Id <- 0
# # define index of id column
# id_idx <- which(colnames(slides_merged_fin@data)=="Id")
# 
# for (n in 1:length(slides_merged_fin)) {
#   slides_merged_fin[n,id_idx] <- n
# }
# export merged slides file
writeOGR(`slides_merged`, dsn = "merged_slides", "merged_slides", driver="ESRI Shapefile", overwrite_layer =  TRUE)
